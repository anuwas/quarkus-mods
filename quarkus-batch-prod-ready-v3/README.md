...
```
stgDB: staging_synch_log.status
...
│    StagingSynchLogRepository.claimChunk()                       │
...
│    ProductRepository.upsertAll()  → mainDB             │
...
The single most important design decision is in `StagingSynchLogRepository.claimChunk()`:

```sql
SELECT id FROM staging_synch_log
...
├── entity/
│   ├── stg/StagingSynchLog.java      stgDB entity with status lifecycle
│   └── main/Product.java             mainDB aggregated output
│        main/BatchExecutionLog.java  per-run audit log
├── repository/
│   ├── stg/StagingSynchLogRepository SELECT FOR UPDATE SKIP LOCKED claim logic
│   └── main/ProductRepository        upsert-aware write
