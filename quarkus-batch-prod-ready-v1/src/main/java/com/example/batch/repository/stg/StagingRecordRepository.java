...
/**
 * Repository for StagingRecord operations on stgDB.
 *
 ...
 */
@ApplicationScoped
public class StagingRecordRepository {
     ...
     * @return list of claimed StagingRecord entities (may be empty if nothing pending)
     */
    @Transactional(Transactional.TxType.REQUIRES_NEW)
    public List<StagingRecord> claimChunk(int chunkSize, String nodeId) {
        // Step 1 — find candidate IDs using keyset + FOR UPDATE SKIP LOCKED
        @SuppressWarnings("unchecked")
        List<Long> ids = em.createNativeQuery(
                "SELECT id FROM staging_record WHERE status = 'PENDING' " +
        ...
        return StagingRecord.list("id IN ?1 AND status = ?2 ORDER BY id ASC",
    ...
    public long countByStatus(RecordStatus status) {
        return StagingRecord.count("status", status);
    }
